...
public class MeshController : MonoBehaviour
{
    ...
    private void CreatePlaneMeshFromTexture(Texture2D texture)
    {
        ...
        // Apply the first material with the base texture
        meshRenderer.sharedMaterial = targetMaterial;
        meshRenderer.sharedMaterial.mainTexture = texture;  // Start with the first texture

        // Apply the overlay texture if available
        if (textureController.overlayTextures != null && textureController.overlayTextures.Length > 0 && textureController.overlayTextures[0] != null)
        {
